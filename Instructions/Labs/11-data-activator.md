...
> **注**: Data Activator のサンプル プロセスにより、バックグラウンドでランダム データが生成されています。  作成する条件とフィルターが複雑になるほど、イベントがトリガー条件やフィルターを満たしていない可能性が高くなります。 グラフにデータが表示されない場合は、数分待ってからページを最新の情報に更新してください。 ラボを続行するために、グラフにデータが表示されるのを待つ必要はありません。
...
1. 既定では、Data Activator によって *Reflex YYYY-MM-DD hh:mm:ss* という名前の Reflex が作成されます。 ワークスペースには複数の Reflex が存在する可能性があるため、既定の Reflex の名前をよりわかりやすい名前に変更する必要があります。 この例では、左上隅にある現在の Reflex 名の横にあるプルダウンを選び、名前を ***Contoso Shipping Reflex*** に変更します。
...
1. 現在 "データ" モードではない場合は、画面左下の **[データ]** タブを選択します。** 実際の例では、EventStreams と Power BI ビジュアルから独自のデータ ソースをここに追加します。 このラボでは、Data Activator によって提供されるサンプル データを使用します。 Data Activator によって提供されるサンプル データには、荷物の配送状態を監視する 3 つの EventStream が既に設定されています。

![Data Activator Reflex データ モードのスクリーンショット。](./Images/data-activator-data-tab.png)

1. 各イベントを選択して、そのイベントで処理されるデータを確認します。
...
1. 上部のメニューで **[新しいトリガー]** ボタンを選択します。 新しいトリガーは、既定の名前の *Untitled* で作成されます。トリガーをより適切に定義するために、名前を ***Medicine temp out of range*** (薬の温度が範囲外) に変更します。

    ![Data Activator Reflex デザインで新しいトリガーを作成するスクリーンショット。](./Images/data-activator-trigger-new.png)

1. 次は、Reflex をトリガーするプロパティまたはイベント列を選択します。 オブジェクトの作成時にいくつかのプロパティを作成したため、 **[既存のプロパティ]** ボタンを選び、***Temperature*** プロパティを選択します。 このプロパティを選択すると、過去の温度値のサンプルを含むグラフが返されます。
...
1. ここまでは、トリガーを起動するプロパティと条件を定義しましたが、必要なすべてのパラメーターが含まれているわけではありません。 *city* が **Redmond** の場合と、*special care* の種類の **Medicine** に対してのみトリガーが起動することを引き続き確認する必要があります。 次は、これらの条件に対していくつかのフィルターを追加してみましょう。  **[フィルターの追加]** ボタンを選択し、***City*** プロパティを選択します。 値として「***Redmond***」を入力します。 次に、 **[フィルターの追加]** ボタンをもう一度選び、***SpecialCare*** プロパティを選択します。 値として「***Medicine***」と入力します。

    ![Data Activator Reflex デザインでフィルターを追加するスクリーンショット。](./Images/data-activator-trigger-select-condition-add-filter.png)

1. 薬が冷蔵されていることを確認するために、フィルターをもう 1 つ追加しましょう。 **[フィルターの追加]** ボタンを選び、***City*** プロパティを選択します。 値として「***Refrigerated***」と入力します。
...
1. メール アクションに次の値を入力します。
    ...
    ![Data Activator Reflex デザインでトリガーを開始するスクリーンショット。](./Images/data-activator-trigger-start.png)
...
トリガーを停止するには、上部のメニューから **[停止]** ボタンを選択します。
