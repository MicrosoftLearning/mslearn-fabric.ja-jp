---
lab:
  title: Fabric IQ のセマンティック モデルからオントロジを構築する (プレビュー)
...
# Fabric IQ のセマンティック モデルからオントロジを構築する (プレビュー)
...
Fabric でオントロジを操作する前に、Fabric 容量を有効にしたワークスペースが必要です。
...
次に、レイクハウスを作成し、オントロジの基礎となる病院の運用データを読み込みます。

1. ワークスペースで、**[+ 新しい項目]**  >  **[レイクハウス]** を選択します。
1. レイクハウスに `LamnaHealthcareLH` と名前を付け、**[作成]** を選択します。
...
1. 5 つのレイクハウス ファイルをアップロードします。
   - レイクハウスで、メイン ビューから **[ファイルのアップロード]** を選択します
   - **Hospitals.csv**、**Departments.csv**、**Rooms.csv**、**Patients.csv**、**VitalSignEquipment.csv** の 5 つのファイルを参照して選択します
   - **[開く]** を選択します
...
1. アップロードされた各ファイルをテーブルに変換します。
   - **[エクスプローラー]** で、**Files** フォルダーを選択すると、5 つの CSV ファイルがすべて表示されます
   - ファイルごとに、ファイル名の右側にある省略記号 **(...)** を選択します
...
1. イベントハウスに `LamnaHealthcareEH` と名前を付け、**[作成]** を選択します。
1. 既定の KQL データベースが、同じ名前で作成されます。 KQL データベースを選択して開きます。
...
1. **[次へ]** を選択し、インジェスト ウィザードを続行して、既定の設定を維持します。
...
6. 時系列バインディングを構成します。
   - **静的セクション** - キーをマップして、ストリーミング データをエンティティにリンクします。
...
1. システムがバックグラウンドでデータを処理している間、"オントロジの更新中" というメッセージが表示されます。 1 - 2 分後に、ブラウザーを更新してエンティティの種類の概要を表示します。

   次のタイルが表示されます。
   - **リレーションシップ グラフ**: このエンティティの種類が他のエンティティの種類に接続する方法の視覚的表現
   - **プロパティ チャート**: プロパティ値の分布を示す横棒グラフ (RoomType、RoomNumber、DepartmentId など)
   - **エンティティ インスタンス テーブル**: プロパティを持つすべての個々の病室インスタンスのリスト

   ![リレーションシップ グラフ、プロパティ チャート、エンティティ インスタンス テーブルを含む Room エンティティの種類の概要を示すスクリーンショット](Images/24-preview-ontology.png)

1. **Entity instances** テーブルで、任意の病室インスタンス (例: **ICU-302**) を選択します。
...
このワークスペースとオントロジを維持して、Fabric IQ の機能を引き続き探索できます。 このモジュールで作成したすべてのリソースを削除するには、次の手順のように操作します。
