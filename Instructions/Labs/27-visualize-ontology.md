...
1. この演習のサンプル データには、2026 年 4 月 1 日のタイムスタンプが付いています。 このラボを後で実行する場合、既定の "過去 30 日間" フィルターでは、この履歴データは表示されません。 バイタル サインの測定値を表示するには、カスタムの日付範囲を設定する必要があります。

1. 時間範囲セレクターで、カスタムの日付範囲を設定します。
   - [**カスタム**] を選択します。
   - **開始日**: 2026 年 4 月 1 日午前 12:00
   - **終了日**: 2026 年 4 月 2 日午前 12:00
   - **細分性:** 5 分
   - **集計**: 平均
   - **[適用]** を選択します

2. 時系列グラフが更新され、指定した時間範囲内のバイタル サインの測定値 (心拍数、酸素飽和度、呼吸数) が表示されるのを観察します。


3. **[Entity instances]** タイルで、機器 ID (**VS-1004** など) を選択して、**[HeartRate]**、**[OxygenSaturation]**、**[RespiratoryRate]** タイルの特定の測定値を表示します。

4. 探索が完了したら、オントロジ キャンバスに戻ります。
