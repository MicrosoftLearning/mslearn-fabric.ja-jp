...
lab:
  ...
  description: 販売地域に基づいてデータ アクセスを制限する静的と動的両方のロールを作成して、Power BI セマンティック モデルに行レベル セキュリティを実装する方法を学びます。 DAX 式を使って、認証されたユーザーに基づいてデータを自動的にフィルター処理する動的ロールを作成した後、セキュリティ プリンシパルをロールにマップしてユーザーがアクセスを認可されたデータのみが表示されるようにします。
  duration: 45 minutes
  level: 300
  islab: true
  primarytopics:
...
* SSMS を介して復元された [AdventureWorksDW2022](https://github.com/MicrosoftLearning/mslearn-fabric/raw/refs/heads/main/Allfiles/Labs/00-Setup/DatabaseBackup/AdventureWorksDW2022.bak) データベース バックアップ
...
このタスクでは、Power BI サービスにサインインし、試用版ライセンスを開始して、[マイ ワークスペース] にアクセスします。
...
4. **[ロールの管理]** ウィンドウで、**[+ 新規]** を選択します。

5. ロールに名前を付けるには、選択した*無題の*テキストを**オーストラリア**に置き換えてから、**Enter** キーを押します。

6. **[テーブルの選択]** リストで **[営業担当地域]** を選択してから、**[ルール]** セクションで **[+ 新規]** を選択します。
  
7. 新しいルール行で、次の設定を設定します。
    * **列**: リージョン
    * **条件**: 次の値と等しい
    * **値**: オーストラリア
    ...
    *このルールは、**Region** 列を値**オーストラリア**でフィルター処理します。*

10. 別のロールを作成するには、**[ロール]** セクションで **[+ 新規]** を押します。

11. このタスクの手順を繰り返して、**Canada** で **Region** 列をフィルター処理する **Canada** という名前のロールを作成します。
...
5. レポートの上部に、適用されたロールを確認する赤いバナーが表示されます。

    ![](Images/enforce-model-security-image25.png)

6. ロールを使用して表示を停止するには、赤いバナーの右側にある **[表示の停止]** を選択します。
...
3. **[Power BI へ発行]** ウィンドウで、**[マイ ワークスペース]** を選んで **[選択]** を選びます。
...
7. ロールのマッピングを完了するには、 **[保存]** を選びます。
    ...
    "*この設計アプローチはシンプルで、地域ごとにセキュリティ グループが存在する場合に効果的です。ただし欠点があります。作成と設定にはより多くの労力が必要です。また、新しい地域が追加 (オンボード) されたときは、セマンティック モデルを更新して再発行する必要もあります。"*
...
5. **[ロールの管理]** ウィンドウで最初のロールを削除するには、その横の省略記号を選択してから、**[削除]** を選択します。
...
5. **[Salesperson]** テーブルを選択し、**[ルール]** タブで **[DAX エディターに切り替える]** を選択します。

    ![](Images/enforce-model-security-image65.png)

6. **[DAX エディター]** ボックスに、次の式を入力します。
...
6. レポートの上部に、適用されたロールを確認する赤いバナーが表示されます。

    ![](Images/enforce-model-security-image73.png)

7. ロールを使用して表示を停止するには、赤いバナーの右側にある **[表示の停止]** を選択します。
