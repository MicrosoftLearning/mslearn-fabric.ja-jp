...
2. **[開発]** ワークスペースを選択します。
3. **[新しい項目]** を選択します。
4. 表示されたウィンドウで **[レイクハウス]** を選択し、**[新しいレイクハウス] ウィンドウ**でレイクハウスの名前を「**LabLakehouse**」に設定します。 **[レイクハウス スキーマ]** チェック ボックスはオンにしたままにします。
5. **［作成］** を選択します
...
8. **[開発]** ステージを選択すると、デプロイ パイプライン キャンバスに、作成したレイクハウスがステージ項目として表示されます。 **[テスト]** ステージの左端の円内に **[X]** マークがあります。 この **[X]** は、[開発] ステージと [テスト] ステージが同期していないことを示します。
9. **[テスト]** ステージを選択すると、デプロイ パイプライン キャンバスで、作成したレイクハウスはソースのステージ項目にすぎず、それがこの場合は **[開発]** ステージを参照していることがわかります。  
...
- 左側のナビゲーション バーで、**[デプロイ パイプライン]** を選択してパイプラインを選択し、設定メニューから **[このパイプラインを削除]** を選択してデプロイ パイプラインを削除します。

![デプロイ パイプラインのスクリーンショット。[パイプラインの削除] アクションが強調表示されています。](./Images/delete-pipeline.png)

- パイプラインを削除した後、各ワークスペースのアイコンを選択して、それに含まれるすべての項目を表示します。
