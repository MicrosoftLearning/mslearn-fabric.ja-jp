...
1. 左側のメニュー バーで、**[作成]** を選択します。 *[新規]* ページの *[データ ウェアハウス]* セクションで、**[サンプル ウェアハウス]** を選択し、**sample-dw** という名前の新しいデータ ウェアハウスを作成します。
    ...
    ![新しいウェアハウスのスクリーンショット。](./Images/06b-sample-data-warehouse.png)
...
運用環境では、範囲外の値や論理エラーなどの異常が発生する可能性があります。 分析前に整合性チェックを実行することをお勧めします。データがクリーンな場合でも、それが役立つ結果であることを確認します。

サンプル データ ウェアハウスで一貫性チェックを実行してみましょう。
...
1. クエリを実行します。 結果が **0** の場合、このデータセットに異常に長い期間のトリップがないことが確認されます。データはこのチェックに合格したことになります。
...
1. 結果が **0** の場合、負の期間のトリップがないことが確認されます。 見つかった場合は、次のようなステートメントで削除できます。
...
1. クエリを変更して `WHERE D.Month = '01'` を追加します。 これにより、1 月のレコードのみを含むようにデータがフィルター処理されます。 最終的なクエリは次のようになるはずです。
...
1. **JanTrip** という名前の新しいビューを作成します。

1. **[エクスプローラー]** で、**[スキーマ] >> [dbo] >> [ビュー]** に移動します。 先ほど作成した *JanTrip* ビューに注意してください。
