...
1. 左側のメニュー バーで、**[作成]** を選択します。 *[新規]* ページの *[Real-Time Intelligence]* セクションで、**[Real-Time ダッシュボード]** を選択し `bikes-dashboard` という名前を付けます。
...
1. ツール バーの **[新しいデータ ソース]** を選択し、**Eventhouse / KQL Database** を選択します。 次に、イベントハウスを選択し、次の設定で新しいデータ ソースを作成します。
    - **表示名**: `Bike Rental Data`
    - **データベース**: *イベントハウス内の既定のデータベース*。
    - **パススルー ID**: *Selected*

1. **[追加]** を選択します。
1. ダッシュボード デザイン キャンバスで、**[タイルの追加]** を選択します。
...
ダッシュボードには現在、すべての近隣の最新の自転車、ドック、場所データが表示されています。 次に、特定の近隣地域を選択できるようにパラメーターを追加します。
...
1. **[完了]** を選択してパラメーターを作成します。
    ...
    パラメーターを追加したので、基本クエリを変更して、選択した近隣地域に基づいてデータをフィルター処理する必要があります。
...
1. ダッシュボードで、**[近隣]** パラメーターを使用して、選択した近隣地域に基づいてデータをフィルター処理します。
...
1. ダッシュボードのツール バーの **[管理]** テーブルで、**[自動更新]** を選択します。
