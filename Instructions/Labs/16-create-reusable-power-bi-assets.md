...
この演習では、セマンティック モデルとレポートの開発をサポートする再利用可能な資産を作成します。 このアセットには、Power BI プロジェクト ファイルやテンプレート ファイル、共有セマンティック モデルが含まれます。 最後に、系列ビューは、これらのアイテムが Power BI サービス内でどのように相互に関連しているかを示します。
...
このタスクでは、再利用可能なテンプレートを作成します。そのテンプレートを共有すれば、他のユーザーも独自のデータ ソースで使用できるようになります。 Power BI テンプレート (.pbit) ファイルは、レポートのレイアウト、ビジュアル、構造を含むテンプレートです。ただし、**データは含まれていません**。 
...
1. このテンプレートを使用した場合にユーザーが期待できることの説明を入力し、[OK] を選択します。
    ...
    > **ヒント**: この説明は、ユーザーが .pbit ファイルを開いたときに表示され、テンプレートの目的と、ユーザーが提供する必要があるデータ ソースを理解するのに役立ちます。

1. エクスプローラーに戻り、*.pbit* ファイルを開きます。

    > **重要**: .pbit ファイルを開くと、テンプレート ファイルにデータが含まれていないため、Power BI によって、データ ソース接続を指定するように求められます。 次の操作を求めるダイアログ ボックスが表示される場合があります。
    > - データ ソースの資格情報を入力する
    > - ファイル パスまたはサーバー接続を指定する
    > - 接続パラメーターを構成する
    >
    > これは予期される動作です。これにより、ユーザーは、埋め込みデータを使用するのではなく、テンプレートを自分のデータ ソースに接続できます。

1. データ ソース ダイアログでメッセージが表示されたら、**[キャンセル]** または **[閉じる]** を選択して、次の手順でデータ ソースを削除します。
1. レポートは、テーマとビジュアルがそのままで、*.pbip* ファイルと同じように見えることに注意してください。
...
1. 以前に使用したものと同じ名前の *.pbit* ファイルで**保存**して、ファイルを上書きします。
    ...
    > **重要なポイント**: これで、真の Power BI テンプレートが作成されました。 この .pbit ファイルには、カスタム テーマとレポート構造が含まれていますが、データは含まれています。 他のユーザーがこのテンプレートを開くと、独自のデータ ソースに接続するように求められるため、次の場合に最適です。
    > - チーム間でレポート デザインを標準化する
    > - データを公開せずにレポート レイアウトを共有する
    > - 一貫したブランド化とテーマで再利用可能なテンプレートを作成する
    > - ユーザーが同じレポート構造を異なるデータセットに適用できるようにする
...
このタスクでは、この演習で実行されたタスクの最終的な結果を示す次のスクリーンショットを確認します。 この状態を実現するため、Power BI プロジェクト ファイルを作成し、ワークスペースに発行しました。 その後、Power BI サービスのワークスペースに移動し、新しいレポートが他のデータ ソースにどのように依存しているかを確認するため、**系列ビュー**に切り替えました。

左から右に、次の項目が表示されます。

- データ ソース: 2 つのテキスト/CSV ファイルと SQL Server 接続。
- 16-Starter-Sales Analysis セマンティック モデル。データ ソースに接続されています。
- 16-Starter-Sales Analysis レポート。16-Starter-Sales Analysis セマンティック モデルに接続されています。
- 新しいレポート セマンティック モデル。16-Starter-Sales Analysis セマンティック モデルに接続されています。
- [新しいレポート] レポート。新しいレポート セマンティック モデルに接続されています。

> セマンティック モデルが他のセマンティック モデルに関連する場合、この状態を**チェーン**といいます。 このラボでは、スターター セマンティック モデルが新しく作成されたセマンティック モデルにチェーンされ、専用の目的での再利用が可能になっています。
