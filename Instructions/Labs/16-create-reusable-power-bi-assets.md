...
lab:
  ...
  description: フラット ファイル ストレージを使用してソース管理を可能にする Power BI Project ファイル (.pbip) と、動的なデータ読み込みを可能にするパラメーター化されたテンプレート ファイル (.pbit) を構築することで、再利用可能な Power BI 資産を作成する方法について学習します。 TMDL 形式を使用して、ソース管理のセマンティック モデルの変更を追跡し、さまざまなデータセットで再利用できるテンプレートを構築します。
  duration: 30 minutes
  level: 300
  islab: true
  primarytopics:
...
1. Power BI Desktop を閉じます。 変更を保存するように求められたら、**[保存しない]** を選択します。
1. `regional-sales.pbit` ファイルを開きます。

    > **注**: サインインを求めるプロンプトが表示されたら、Microsoft 組織のアカウント資格情報を入力します。 プライバシー レベルのダイアログが表示される場合は、**[このファイルのプライバシー レベルのチェックを無視します]** を選択して、**[保存]** を選択します。
...
1. **[読み込み]** を選択してデータを読み込み、レポートを開きます。
