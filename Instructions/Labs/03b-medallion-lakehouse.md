...
lab:
  ...
  description: PySpark と Delta Lake を使用して生データをブロンズ、シルバー、ゴールドの各レイヤーを通して変換するという方法でメダリオン アーキテクチャを実装します。 ディメンションとファクト テーブルを持つスター スキーマを作成してから、データを探索して分析とレポートのためにモデル化します。
  duration: 45 minutes
  level: 300
  islab: true
  primarytopics:
