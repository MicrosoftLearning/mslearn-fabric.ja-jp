...
lab:
  ...
  description: このラボでは、Eventstream を使って株価データを取り込み、KQL を使ってそのクエリを実行し、リアルタイム ダッシュボードで視覚化することで、Microsoft Fabric でリアルタイム データ ストリームを操作する方法を学びます。 また、Activator を使ってリアルタイム イベントに応答するようにアラートを構成します。
  duration: 30 minutes
  level: 300
  islab: true
  primarytopics:
...
> **ヒント:** 関連するトレーニング コンテンツについては、「[Microsoft Fabric のリアルタイム インテリジェンスの使用を開始する](https://learn.microsoft.com/training/modules/get-started-kusto-fabric/)」をご覧ください。
...
これで、ストリーミング ソースからリアルタイム データを検索して取り込む準備ができました。 これを行うには、Fabric リアルタイム ハブから開始します。 リアルタイム ハブを使用すると、ストリーミング データのソースを簡単に見つけて管理できます。

> **ヒント**: 初めてリアルタイム ハブを使うときは、*作業開始*のヒントが表示される場合があります。 これらを閉じることができます。

1. 左側のメニュー バーで、**リアルタイム** ハブを選択します。

    > **注**: **[リアルタイム ハブ]** が表示されない場合は、省略記号 **(...)** を選んで、リアルタイム ハブをメニュー バーにピン留めします。

    ![リアルタイム ハブをメニュー バーにピン留めするスクリーンショット。](./Images/pin-real-time-hub.png)

1. **[リアルタイム ハブ]** で **[データの追加]** を選びます。

    ![リアルタイム ハブのスクリーンショット。](./Images/real-time-hub.png)

2. **Stock market** サンプル データ ソースを選びます。

3. データ ソースを次のように構成します。
   - **ソース名**: `stock`
   - **ワークスペース**: 作成したワークスペースを選びます
   - **Eventstream 名**: `stock-data`

    > _このデータに関連付けられる既定のストリームには、*stock-data-stream* という名前が自動的に付けられます。_

    ![新しい Eventstream のスクリーンショット。](./Images/name-eventstream.png)

4. **[次へ]** を選んでから、**[接続]** を選んで Eventstream を作成します。 

5. **[Eventstream を開く]** を選びます。 イベントストリームには、デザイン キャンバスに**stock**ソースと **stock-data-stream** が表示されます。
