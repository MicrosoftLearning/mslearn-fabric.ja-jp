...
1. **ワークスペース**に移動し、**[+ 新しいアイテム]** を選択し、「**ノートブック**」を検索して新しいノートブックを作成します。

    数秒後に、1 つの ''セル'' を含む新しいノートブックが開きます。** ノートブックは、''コード'' または ''マークダウン'' (書式設定されたテキスト) を含むことができる 1 つ以上のセルで構成されます。** **

1. 左側の **[エクスプローラー]** ペインで、**[データ項目の追加] > [OneLake カタログ]** を選択し、レイクハウスを追加します。

1. ノートブックから、ノートブック内の既存のセルを選択し、既定のコードを次の変数宣言に置き換えます。
