...
lab:
  ...
  description: このラボでは、Microsoft Fabric レイクハウスを作成してデータをインポートします。 レイクハウスによってどのようにファイルとテーブル ストレージが OneLake 内で結合されるかを探索するとともに、マネージド デルタ テーブルに対するクエリを SQL を使用して実行する方法を学習します。
  duration: 30 minutes
  level: 200
  islab: true
  primarytopics:
...
> **ヒント:** 関連するトレーニング コンテンツについては、「[Microsoft Fabric でレイクハウスを始める方法](https://learn.microsoft.com/training/modules/get-started-lakehouses/)」を参照してください。
...
1. **sales.csv** ファイルの **[...]** メニューで、**[テーブルに読み込む]** > 、**[新しいテーブル]** の順に選択します。
1. **[テーブルに読み込む]** ダイアログ ボックスで、テーブル名を `sales` に設定して読み込み操作を確定します。 その後、テーブルが作成されて読み込まれるのを待ちます。
1. ファイルの種類は **CSV** を選択します。 その後、テーブルが作成されて読み込まれるのを待ちます。

    > **ヒント**: `sales` テーブルが自動的に表示されない場合は、**[Tables]** フォルダーの **[...]** メニューで **[最新の情報に更新]** を選択します。

3. **[エクスプローラー]** ペインで、作成された `sales` テーブルを選択するとデータを見ることができます。

    ![テーブル プレビューのスクリーンショット。](./Images/table-preview.png)

4. `sales` テーブルの **[...]** メニューで、**[ファイルの表示]** を選択すると、このテーブルの基になるファイルを見ることができます
...
2. 開いた新しいビジュアル クエリ エディター ペインに `sales` テーブルをドラッグすると、次のように Power Query が作成されます。 
...
この演習では、レイクハウスを作成し、それにデータをインポートしました。 レイクハウスが OneLake データ ストアに格納されているファイルとテーブルで構成されるしくみを確認しました。 マネージド テーブルに対しては、SQL を使ってクエリを実行できます。
