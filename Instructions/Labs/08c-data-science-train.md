...
1. データは Spark データフレームとして読み込まれます。 Scikit-learn では、入力データセットが Pandas データフレームであることが期待されます。 データセットを Pandas データフレームに変換するには、次のコードを実行します。
...
ログに記録された estimator ごとのパフォーマンス メトリックをプロットすることで、どのアルゴリズムがより優れたモデルにつながったかを確認できます。
