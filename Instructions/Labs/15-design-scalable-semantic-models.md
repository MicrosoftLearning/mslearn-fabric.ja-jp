...
lab:
  ...
  description: このラボでは、DAX を使って柔軟でスケーラブルなセマンティック モデルを設計する高度な手法を学びます。 非アクティブなリレーションシップを操作し、動的なタイム インテリジェンスのための計算グループを作成し、複数の視覚エフェクトを必要とせずにユーザーが異なるメジャーと計算を切り替えることができる対話型レポートを、フィールド パラメーターを使って作成します。
  duration: 30 minutes
  level: 300
  islab: true
  primarytopics:
